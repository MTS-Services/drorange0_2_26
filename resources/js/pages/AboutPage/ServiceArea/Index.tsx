...
export default function Index({ items, pagination, offset, filters, search, sortBy, sortOrder }: Props) {
  ...
  const actions: ActionConfig<Item>[] = [
    { label: 'Show', icon: <Eye className="h-4 w-4" />, onClick: (item) => router.visit(route('admin.pm.about-service-area.show', item.id)) },
    { label: 'Edit', icon: <Pencil className="h-4 w-4" />, onClick: (item) => router.visit(route('admin.pm.about-service-area.edit', item.id)) },
    {
      label: 'Delete',
      icon: <Trash2 className="h-4 w-4" />,
      onClick: (item) => {
        if (confirm(`Are you sure you want to delete ${item.title}?`)) {
          router.delete(route('admin.pm.about-service-area.destroy', item.id));
        }
      },
      variant: 'destructive',
    },
  ...
  return (
    <AdminLayout activeSlug="about-service-area">
      <Head title="About - Service Areas" />

      <div className="flex justify-end mb-6">
        <Link href={route('admin.pm.about-service-area.create')}>
