...
export default function Index({ items, pagination, offset, filters, search, sortBy, sortOrder }: Props) {
  ...
  const columns: ColumnConfig<Item>[] = [
    { key: 'title', label: 'Title', sortable: true, render: (item) => <div className="font-medium">{item.title}</div> },
    {
      key: 'subtitle',
      label: 'Subtitle',
      sortable: true,
      render: (item) => <div className="text-sm text-gray-700 dark:text-gray-300">{item.subtitle}</div>,
    },
    {
      key: 'icon',
      label: 'Icon',
      ...
      render: (item) => (
        <div className="font-medium text-gray-900 dark:text-gray-100">
          {item.icon_url ? <img src={item.icon_url} alt={item.title} className="h-10 w-10 object-contain" /> : '—'}
        </div>
      ),
    },
    {
  ...
  const actions: ActionConfig<Item>[] = [
    { label: 'Show', icon: <Eye className="h-4 w-4" />, onClick: (item) => router.visit(route('admin.pm.about-why-choose.show', item.id)) },
    { label: 'Edit', icon: <Pencil className="h-4 w-4" />, onClick: (item) => router.visit(route('admin.pm.about-why-choose.edit', item.id)) },
    {
      label: 'Delete',
      icon: <Trash2 className="h-4 w-4" />,
      onClick: (item) => {
        if (confirm(`Are you sure you want to delete ${item.title}?`)) {
          router.delete(route('admin.pm.about-why-choose.destroy', item.id));
        }
      },
      variant: 'destructive',
    },
  ...
  return (
    <AdminLayout activeSlug="about-why-choose">
      <Head title="About - Why Choose" />

      <div className="flex justify-end mb-6">
        <Link href={route('admin.pm.about-why-choose.create')}>
