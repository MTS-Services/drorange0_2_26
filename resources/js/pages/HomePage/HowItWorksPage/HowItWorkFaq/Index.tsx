...
export default function Index({ items, pagination, offset, filters, search, sortBy, sortOrder }: Props) {
  ...
  const actions: ActionConfig<Item>[] = [
    {
      ...
      onClick: (item) => router.visit(route('admin.pm.how-it-work-faq.show', item.id)),
    },
    {
      label: 'Edit',
      icon: <Pencil className="h-4 w-4" />,
      onClick: (item) => router.visit(route('admin.pm.how-it-work-faq.edit', item.id)),
    },
    {
      label: 'Delete',
      ...
      onClick: (item) => {
        if (confirm(`Are you sure you want to delete ${item.title}?`)) {
          router.delete(route('admin.pm.how-it-work-faq.destroy', item.id));
        }
      },
      variant: 'destructive',
    },
  ...
  return (
    <AdminLayout activeSlug="how-it-work-faq">
      <Head title="How It Work FAQ" />

      <div className="flex justify-end mb-6">
        <Link href={route('admin.pm.how-it-work-faq.create')}>
