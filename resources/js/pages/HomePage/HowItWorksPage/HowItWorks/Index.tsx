...
export default function Index({ items, pagination, offset, filters, search, sortBy, sortOrder }: Props) {
  ...
  const columns: ColumnConfig<Item>[] = [
    ...
    },
    {
      ...
      label: 'Time',
      sortable: true,
      render: (item) => <div className="font-medium text-gray-900 dark:text-gray-100">{item.time}</div>,
    },
    {
      key: 'icon',
      label: 'Icon',
  ...
  const actions: ActionConfig<Item>[] = [
    {
      ...
      onClick: (item) => router.visit(route('admin.pm.how-it-works.show', item.id)),
    },
    {
      label: 'Edit',
      icon: <Pencil className="h-4 w-4" />,
      onClick: (item) => router.visit(route('admin.pm.how-it-works.edit', item.id)),
    },
    {
      label: 'Delete',
      ...
      onClick: (item) => {
        if (confirm(`Are you sure you want to delete ${item.title}?`)) {
          router.delete(route('admin.pm.how-it-works.destroy', item.id));
        }
      },
      variant: 'destructive',
    },
  ...
  return (
    <AdminLayout activeSlug="how-it-works">
      ...
      <div className="flex justify-end mb-6">
        <Link href={route('admin.pm.how-it-works.create')}>
