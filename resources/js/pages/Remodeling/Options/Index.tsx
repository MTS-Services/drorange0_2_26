...
export default function Index({ options, pagination, offset, filters, search, sortBy, sortOrder }: Props) {
  const { isLoading, handleSearch, handleFilterChange, handleSort, handlePerPageChange, handlePageChange } = useDataTable();

  const columns: ColumnConfig<OptionItem>[] = [
    {
      key: 'title',
      label: 'Title',
      sortable: true,
  ...
  const actions: ActionConfig<OptionItem>[] = [
    {
      label: 'Show',
      icon: <Eye className="h-4 w-4" />,
      onClick: (item) => router.visit(route('admin.pm.remodeling-option.show', item.id)),
    },
    {
      label: 'Edit',
      icon: <Pencil className="h-4 w-4" />,
      onClick: (item) => router.visit(route('admin.pm.remodeling-option.edit', item.id)),
    },
    {
      label: 'Delete',
      ...
      onClick: (item) => {
        if (confirm(`Are you sure you want to delete ${item.title}?`)) {
          router.delete(route('admin.pm.remodeling-option.destroy', item.id));
        }
      },
      variant: 'destructive',
    },
  ...
  return (
    <AdminLayout activeSlug="remodeling-option">
      <Head title="Remodeling Options" />

      <div className="flex justify-end mb-6">
        <Link href={route('admin.pm.remodeling-option.create')}>
